...
async def login(req: LoginRequest, request: Request, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    # refresh_token 不用传给前端/users/refresh
    refresh_token = None
...
async def logout(
    ...
    # 查找 refresh_token
    stmt = select(RefreshToken).where(RefreshToken.jti == token_uuid)
    result = await db.execute(stmt)
    refresh_token = result.scalar_one_or_none()
    if not refresh_token:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="refresh_token不存在")

    # 标记为已注销
    upd = update(RefreshToken).where(RefreshToken.jti == token_uuid).values(is_revoked=True)
    await db.execute(upd)
    await db.commit()

    # 返回响应，前端收到后应删除本地 access_token
    return LogoutResponse(message="注销成功，refresh token 已经注销")
